...
var SCEPlugin = (function() {
  ...
  SCEPlugin.prototype.initialize = function() {
    ...
    this.currentNode = this.currentNode.setLeftMost(
      data_1.values["driver.create"].copy()
    );
  ...
  SCEPlugin.prototype.onNextEvent = function(m) {
    ...
    if (String(m.eventName) === "sumanload") {
      this.currentNode = this.currentNode.setRightMost(
        data_1.values.before.copy().render()
      );
      this.currentNode = this.currentNode.setLeftMost(
        data_1.values["driver.get"].copy().render({ driverGetURL: m.location.href })
      );
      this.inHook = true;
    }
    if (String(m.eventName) === "click") {
      ...
      this.currentNode = this.currentNode.setLeftMost(
        data_1.values["driver.find.hook"].copy().render({ driverGetURL: m.location.href })
      );
      this.inHook = true;
    }
    if (this.inHook) {
